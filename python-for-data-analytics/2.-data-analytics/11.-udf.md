...
{% file src="../../.gitbook/assets/claim.csv" %}

UDF (user-defined function) is a function written by a user to apply custom logic to your data.
...
5. Create a UDF that classifies orders:
   * Quantity ≥ 3 → High Order
   * Otherwise → Low Order
6. Create a UDF that creates price bands:
   * <100 → Budget
   * 100–300 → Mid Range
   * > 300 → Premium
7. Create a UDF that extracts the numeric part from Customer ID (CUST014 → 14) and returns the number of digits.

***

#### **Level 3 – Row Based**

8. Create a UDF to calculate a Business Score:
   * Score = (Age / 2) + (Quantity × 5)
9. Create a UDF that assigns product appeal:
...
11. Create a UDF to extract only the numeric portion from Customer ID (CUST009 → 9).
12. Create a UDF to mask Customer ID so only the first two characters are visible (CUST009 → CU\*\*\*\*).

***

#### **Level 5 – Business Logic**

13. Based on Total Amount, create a UDF to assign Purchase Tier:

* <500 → Bronze
* 500–1500 → Silver
* > 1500 → Gold

14. Create a UDF to assign discount percentage:

* Age < 25 → 10%
* Price per Unit > 300 → 15%
* Otherwise → 5%

15. Create a UDF to calculate Final Bill:

* Final Bill = Total – (Total × Discount%)

### On Insurance Data

1. Create a UDF that assigns Age Category based on Customer\_Age:
   * <25 → Young
   * 25–45 → Adult
   * > 45 → Senior
2. Create a UDF that assigns Vehicle Category:
   ...
   * 5–10 → Mid Age
   * > 10 → Old
3. Create a UDF that converts Location\_Risk into a score:
   * High → 3
   * Medium → 2
   * Low → 1
...
4. Create a UDF that checks Accident Risk:
   * Accidents\_Last\_3Y = 0 → Safe Driver
   * 1–2 → Moderate Risk
   * > 2 → High Risk
5. Create a UDF that classifies claim history:
   * Claim\_History (yrs) = 0 → No Claim History
   * 1–5 → Recent Claims
   * > 5 → Long Claim History
6. Create a UDF that assigns a flag:
   * Vehicle Age > 10 AND Location\_Risk = High → Critical Vehicle
   * Otherwise → Normal
...
7. Create a UDF to calculate a “Customer Stability Score”:\
   Formula example:
   * Score = Customer\_Age – (Accidents\_Last\_3Y × 5)
8. Create a UDF to calculate “Risk Index” considering:
   * Higher age adds stability
   * More vehicle age increases risk
   * More accidents increase risk
9. Create a UDF that assigns:
   * High Priority Claim if:
     * Vehicle\_Age > 10 OR Accidents\_Last\_3Y > 2
   * Otherwise Regular
...
10. Create a UDF to calculate claim settlement amount factor:

* Young + High location risk → factor 1.25
* Old + Low risk → factor 0.85
* Others → factor 1.0

11. Create a UDF that determines premium category:

* High risk accident + high location → Premium+
* Medium combinations → Standard
* Otherwise → Basic

12. Create a UDF to decide fraud suspicion:

* If Claim\_History (yrs) > 7 AND Accidents\_Last\_3Y > 2 → Suspicious
...
13. Create a UDF that generates a composite Insurance Score:\
    Scoring must use:
...
14. Create a UDF that assigns Final Grade based on score:

* > 80 → A
* 60–80 → B
* <60 → C

15. Create a UDF to calculate Renewal Probability:
...
16. Create a UDF to identify:

* High Mileage Case if vehicle age > 12 AND customer age < 35

17. Create a UDF to detect:

* Age-Vehicle Conflict:
  * Customer > 60 AND vehicle age < 3 → Flag “Unusual Case”
  * Otherwise Normal

18. Create a UDF that returns a text summary sentence for each row, like:\
    “Driver is 45 years old with 2 accidents in last 3 years and lives in a Low risk area.”
