...
In this project, we are going to analyze 12 months of sales data. We will learn how to clean, manage, and analyze the dataset to find some meaningful information.

We start by cleaning our data. Tasks during this section include:

* [x] <mark style="color:purple;">**Drop NaN values from the DataFrame**</mark>
...
* [x] <mark style="color:purple;">**What city sold the most products?**</mark>
* [x] <mark style="color:purple;">**What time should we display advertisements to maximize the likelihood of customers buying the product?**</mark>
...
```python
base_path = r'C:\Users\abhis\Downloads\SalesData'
```
...
**Let us go through the one-month data first, to know things better.**
...
This is how our one-month data looks. Can we get all the files from the directory?

### <mark style="color:green;">List all files from the directory</mark>

To get all files from a specific directory, we can use the **listdir() method.**
...
Now we need to concatenate all these files into a single file, for that we need to create an empty DataFrame and concatenate all sales files.
...
Let us read out the data from the updated DataFrame.
...
To convert the Order Date into a Datetime, the first thing that needs to be done is to bring all dates into a single format.

<pre class="language-python"><code class="lang-python">all_data['Order Date'] = all_data['Order Date'].str.replace('/','-')
<strong>all_data['Order Date'] = pd.to_datetime(all_data['Order Date'])
</strong></code></pre>
...
Now, let us try to convert the Order Date again.
...
To add a month column, all we need to do is use date time methods.
...
Now _**Price Each, Quantity Ordered**_ is an object data type, so we need to convert it.
...
Now we can easily use the **groupby** method to find the maximum sales value.
...
#### <mark style="color:green;">**What city sold the most products?**</mark>

There is no City Column in our Dataset, but you can see the Purchase Address, where the city is mentioned in the middle. We can use the purchase address and create a City Column.
...
Now we can group by City and calculate the total quantity ordered.
...
Because we need to work on time, let's create an **Hour** and **Minute** Column.
...
What Products are most often sold together gives a very important insight that helps the company lure customers into spending more and generate more profit.
...
Same Order ID for different products suggests the customer bought different products together in Our Data. Let us look for Duplicate OrderId Products:
...
This is pretty simple; we will group all products and calculate the quantity ordered.
