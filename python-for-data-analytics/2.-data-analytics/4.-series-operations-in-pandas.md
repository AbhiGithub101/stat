...
{% embed url="https://youtu.be/FpWGGm0kx6A?si=xiYLDmUYNC1e8_QF" %}
...
4. Access top 5 rows of Retailer , Product and Price column records from the dataframe.
...
```python
mdf=df[['Retailer','Product','Price per Unit']]
mdf.head() # it will give only top 5 rows by default
```
...
```python
df['Total price']=df['Price per Unit']*df['Units Sold']
df
```
...
```python
df['Serial_no.']=np.arange(1,df.shape[0]+1)
df
```
...
```python
df['discount']=np.random.randint(1,5,df.shape[0])
df
```
