...
<pre class="language-py"><code class="lang-py"><strong>total_score=np.sum(arr) # Total score
</strong><strong>maximum_score=np.max(arr) # Maximum score
</strong><strong>minimum_score=np.min(arr) # Minimum score
</strong><strong>mean_score=np.mean(arr) # Mean Score
</strong><strong>total_score,maximum_score,minimum_score,mean_score
</strong></code></pre>
...
```py
random_int=np.random.randint(1,100,20) # will generated 20 random integers between 1 to 100
random_int
```
...
```py
random_float=np.random.uniform(1,100,20) # will generated 20 random floats between 1 to 100
random_float
```
...
```py
d = {
  'name': ['abhishek','ayush','aman','shikher','suraj'],
  'height': np.random.uniform(100, 200, 5),
  'weight': np.random.uniform(50, 90, 5),
  'age':np.random.randint(1, 100, 50),
  'hair_color': np.random.choice(['black','white'], 5)
}
print(d)
```
