...
{% embed url="https://youtu.be/Gh-icVwAJjk?si=y_0vWrG0sHw1c2J5" %}
...
3. Filter transactions from **2024**.
...
```python
df.loc[df['date'].between('2024-01-01', '2024-12-31')]
```

</details>

4. Filter transactions from **March**.
...
```python
df.loc[df['date'].dt.month == 3]
```

</details>

5. Find transactions that happened on **Fridays**.
...
```python
df.loc[df['date'].dt.day_name() == 'Friday']
```

</details>

6. Find the **total amount spent per month**.
...
```python
df.groupby(df['date'].dt.month).agg(total_amount=('amount', 'sum'))
```

</details>

7. Find the **average amount per category for each year**.
...
```python
df.groupby([df['date'].dt.year, 'category']).agg(avg_amount=('amount', 'mean'))
```

</details>

8. Filter transactions from the **first quarter**.
...
```python
df.loc[df['date'].dt.quarter == 1]
```

</details>

#### expense\_datetime.csv
...
2. Extract the **year**, **month**, **day**, **hour**, **minute**, and **second**.
...
```python
df.loc[df['date'].dt.year == 2024]
```

</details>

4. Filter transactions from **January**.
...
5. Filter transactions **after 6 PM**.
...
```python
df.loc[df['date'].dt.hour >= 18]
```

</details>

6. Find the **total spending per month**.
...
```python
df.groupby([df['date'].dt.year, df['date'].dt.month]).agg(
...
7. Find the **average spending per category per year**.
...
```python
df.groupby([df['date'].dt.year, 'category']).agg(
...
1. Convert both delivery-time columns to datetime.
...
2. Extract the **date**, **hour**, **minute**, and **second** from the promised time.
...
```python
df['date'] = df['promised_delivery_time'].dt.date
df['hour'] = df['promised_delivery_time'].dt.hour
df['minute'] = df['promised_delivery_time'].dt.minute
df['second'] = df['promised_delivery_time'].dt.second
```
...
```python
df.loc[df['promised_delivery_time'].dt.year == 2024]
```

</details>

4. Filter deliveries promised in the **morning**.
...
```python
df.loc[df['promised_delivery_time'].dt.hour < 12]
```

</details>

5. Find deliveries where both times are **exactly the same**.
...
```python
df.loc[df['actual_delivery_time'] == df['promised_delivery_time']]
```

</details>

6. Calculate the difference between actual and promised delivery times.
...
```python
df['delivery_difference'] = df['actual_delivery_time'] - df['promised_delivery_time']
df['delivery_difference_minutes'] = df['delivery_difference'].dt.total_seconds() / 60
```

</details>

7. Find **late** and **early** deliveries.
...
```python
late = df.loc[df['delivery_difference_minutes'] > 0]
early = df.loc[df['delivery_difference_minutes'] < 0]
```

</details>

8. Calculate the **average delay or early time**.
...
```python
df['delivery_difference_minutes'].mean()
```

</details>

9. Find the **maximum delay** and its delivery time.
...
```python
df.loc[df['delivery_difference_minutes'].idxmax()]
```

</details>

10. Find the **minimum delay** and its delivery time.
...
```python
df.loc[df['delivery_difference_minutes'].idxmin()]
```

</details>

11. Count deliveries that were **on time**.
...
```python
(df['delivery_difference_minutes'] == 0).sum()
```

</details>

12. Count deliveries that were late by **more than 5 minutes**.
...
```python
(df['delivery_difference_minutes'] > 5).sum()
```
...
2. Extract date and time components from `order_time`.
...
3. Filter orders from a specific year.
...
```python
df.loc[df['order_time'].dt.year == 2025] # to get only for the year 2025
```

</details>

4. Filter orders from a specific month.
...
```python
df.loc[df['order_time'].dt.month == 1] # to get only for the month january
```

</details>

5. Find orders where both timestamps are **exactly the same**.
...
```python
df.loc[df['delivery_time'] == df['order_time']]
```
