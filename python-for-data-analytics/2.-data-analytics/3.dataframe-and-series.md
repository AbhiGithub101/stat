...
By default , describe() gives descriptive statistics for only Numerical columns but we can pass include='all' parameter to get descriptive statistics for all columns
...
```python
df.tail() # By default it will give bottom 5 rows
```
...
```python
summary=df.descibe()
summary.to_excel('Summary.xlsx') # To save in excel file
summary.to_csv('Summary.csv') # To save in csv file
```
