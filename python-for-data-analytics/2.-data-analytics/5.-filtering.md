...
```python
df.loc[df['Price per Unit'] == 50.00]
```
...
```python
df.loc[df['Sales Method'] == 'In-store']
```
...
```python
df.loc[(df['Retailer'] == 'Foot Locker') & (df['City'] == 'New York')]
```
...
```python
df.loc[df['Units Sold'].between(900, 1200)]
```
...
```python
df.loc[(df['Region'] == 'Northeast') & (df['State'] == 'New York')]
```
...
```python
df.loc[(df['Units Sold'] > 1000) & (df['Price per Unit'] > 45.00)]
```
...
```python
df.loc[(df['Units Sold'] > 800) & (df['Units Sold'] < 1200)]
```
...
```python
df.loc[(df['Price per Unit'] == 50.00) & (df['Sales Method'] == 'In-store')]
```
...
```python
df.loc[df['Units Sold'].isin([850, 900])]
```
...
```python
df.loc[~df['Units Sold'].isin([850, 900, 1250])]
```
...
```python
df.loc[df['Product'].isin(["Men's Apparel", "Women's Apparel"])]
```
...
```python
df.loc[(df['City'] == 'New York') & (df['State'] == 'New York')]
```
...
```python
df.loc[(df['Price per Unit'] == 45.00) | (df['Price per Unit'] == 50.00)]
```
...
```python
df.loc[df['Region'].isin(['Northeast', 'Southwest'])]
```
...
```python
df.loc[df['Units Sold'].between(1000, 1200)]
```
...
```python
df.loc[df['Price per Unit'].between(45.00, 60.00)]
```
...
```python
df.loc[df['Sales Method'].isin(['In-store', 'Online'])]
```
...
```python
df.loc[~df['Region'].isin(['Northwest', 'Midwest'])]
```
...
```python
df.loc[~df['Units Sold'].between(900, 1100)]
```
...
```python
df.loc[(df['Price per Unit'].between(40.00, 50.00)) & (df['Units Sold'] > 900)]
```
...
```python
df.loc[df['Units Sold'].isin([850, 1000, 1200, 1250])]
```
...
30. Use `.loc` to select rows where `Price per Unit` > `$45.00`.
...
```python
df.loc[df['Price per Unit'] > 45.00]
```

</details>

31. Use `.loc` to select rows where `Units Sold` < 1000 **and** `Product` is `"Men's Street Footwear"`.
...
```python
df.loc[(df['Units Sold'] < 1000) & (df['Product'] == "Men's Street Footwear")]
```

</details>

32. Using `.loc`, filter rows where `Sales Method` is `"In-store"` and `Region` is `"Northeast"`.
...
```python
df.loc[(df['Sales Method'] == 'In-store') & (df['Region'] == 'Northeast')]
```

</details>

33. Using `.loc`, select rows where `Units Sold` > 1000 **or** `Product` is `"Women's Apparel"`.
...
```python
df.loc[(df['Units Sold'] > 1000) | (df['Product'] == "Women's Apparel")]
```

</details>

34. With `.loc`, exclude rows where `Price per Unit` equals `$50.00`.
...
35. Update `Sales Method` to `"Online"` where `Units Sold` > 1200.
...
```python
df.loc[df['Units Sold'] > 1200, 'Sales Method'] = 'Online'
```

</details>

36. Change `Price per Unit` to `$55.00` for all `"Men's Apparel"` products.
...
```python
df.loc[df['Product'] == "Men's Apparel", 'Price per Unit'] = 55.00
```

</details>

37. Set `Region` to `"East"` for all rows where `State` is `"New York"`.
...
```python
df.loc[df['State'] == 'New York', 'Region'] = 'East'
```

</details>

38. Update multiple columns — set `Price per Unit = "$60.00"` and `Sales Method = "Online"` where `Units Sold` > 1000.
...
```python
df.loc[df['Units Sold'] > 1000, ['Price per Unit', 'Sales Method']] = [60.00, 'Online']
```
