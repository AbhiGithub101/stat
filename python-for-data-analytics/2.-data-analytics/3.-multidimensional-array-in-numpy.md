# 3. Multidimensional Array in Numpy

## Dimensions in an Array

In this session we are going to cover dimensions in arrays. A dimension in an array is how many levels of indexing you need to reach an element.

```python
import numpy as np
```

### What is a 1-dimensional array?

A simple list — only one index is needed to access an element.

```python
a = [10, 20, 30]
arr = np.array(a)
arr[1]
```

Output:

```
np.int64(20)
```

Only one index is needed to reach a value. This is a single dimensional array. You can check number of dimensions of any array by `ndim`.

```python
arr.ndim
```

Output:

```
1
```

### What is a 2-dimensional array?

A nested list (lists in a list) — two indices are needed to access an element.

```python
b = [[10, 20, 30], [40, 50, 60]]
arr = np.array(b)
arr
```

Output:

```
array([[10, 20, 30],
       [40, 50, 60]])
```

```python
arr[0][0]
```

Output:

```
np.int64(10)
```

```python
arr[0][1]
```

Output:

```
np.int64(20)
```

### How to access a single row in a 2-dimensional array

The first index indicates the row and the second index indicates the column.

```python
arr[0]
```

Output:

```
array([10, 20, 30])
```

### How to access a single column

To access a column, you must give the row also (two indices).

```python
arr[0, 0]
```

Output:

```
np.int64(10)
```

```python
arr[0, 2]
```

Output:

```
np.int64(30)
```

Note: The first value is the row and the second value is the column.

### How to perform slicing in a row

```python
b = [[10, 20, 30],
     [40, 50, 60],
     [70, 80, 90],
     [30, 60, 50],
     [60, 70, 90]]
arr = np.array(b)
arr
```

Output:

```
array([[10, 20, 30],
       [40, 50, 60],
       [70, 80, 90],
       [30, 60, 50],
       [60, 70, 90]])
```

#### Get starting 3 rows (indices 0, 1, 2)

You can slice rows from your array.

```python
arr[0:3]
```

Output:

```
array([[10, 20, 30],
       [40, 50, 60],
       [70, 80, 90]])
```

#### Get last 3 rows (indices 2, 3, 4)

```python
arr[2:5]
```

Output:

```
array([[70, 80, 90],
       [30, 60, 50],
       [60, 70, 90]])
```

Or:

```python
arr[2:]
```

Output:

```
array([[70, 80, 90],
       [30, 60, 50],
       [60, 70, 90]])
```

### Get starting 2 rows and starting 2 columns

```python
arr[0:2, 0:2]
```

Output:

```
array([[10, 20],
       [40, 50]])
```

### Get starting 2 rows and last 2 columns

```python
arr[0:2, 1:3]
```

Output:

```
array([[20, 30],
       [50, 60]])
```

Or:

```python
arr[0:2, 1:]
```

Output:

```
array([[20, 30],
       [50, 60]])
```

## Basic functions in multidimensional arrays

```python
b = [[10, 20, 30],
     [40, 50, 60],
     [70, 80, 90],
     [30, 60, 50],
     [60, 70, 90],
     [90, 70, 20],
     [20, 50, 70]]
arr = np.array(b)
arr
```

Output:

```
array([[10, 20, 30],
       [40, 50, 60],
       [70, 80, 90],
       [30, 60, 50],
       [60, 70, 90],
       [90, 70, 20],
       [20, 50, 70]])
```

### sum

It will add all the values in an array.

```python
np.sum(arr)
```

Output:

```
np.int64(1130)
```

### sum(axis=0) for columns

Here axis = 0 means operate along columns (i.e., sum per column).

```python
np.sum(arr, axis=0)
```

Output:

```
array([320, 400, 410])
```

### sum(axis=1) for rows

```python
np.sum(arr, axis=1)
```

Output:

```
array([ 60, 150, 240, 140, 220, 180, 140])
```

### max(axis=0) for columns

```python
np.max(arr, axis=0)
```

Output:

```
array([90, 80, 90])
```

### max(axis=1) for rows

```python
np.max(arr, axis=1)
```

Output:

```
array([30, 60, 90, 60, 90, 90, 70])
```

### min(axis=0) for columns

```python
np.min(arr, axis=0)
```

Output:

```
array([10, 20, 20])
```

### mean(axis=1) for rows

```python
np.mean(arr, axis=1)
```

Output:

```
array([20.        , 50.        , 80.        , 46.66666667,
       73.33333333, 60.        , 46.66666667])
```

### mean(axis=0) for columns

```python
np.mean(arr, axis=0)
```

Output:

```
array([45.71428571, 57.14285714, 58.57142857])
```

##

### Assignments

1. Create a Multidimensional array of 10 rows and 4 columns.

<details>

<summary>Solution</summary>

<pre class="language-py"><code class="lang-py"><strong>arr = [
</strong>    [101, 85, 90, 88],
    [102, 78, 82, 75],
    [103, 92, 89, 95],
    [104, 67, 70, 72],
    [105, 88, 84, 91],
    [106, 74, 79, 77],
    [107, 81, 85, 80],
    [108, 95, 93, 97],
    [109, 69, 71, 68],
    [110, 90, 87, 92]
]
arr=np.array(arr)
arr
</code></pre>

</details>

2. Get the first two rows and last three columns from the array

<details>

<summary>Solution</summary>

```python
arr[0:2,1:]
```

</details>

3. Get the last two rows and first three columns from the array

<details>

<summary>Solution</summary>

```python
arr[8:,0:3]
```

</details>

4. Get the first column from the array

<details>

<summary>Solution</summary>

```
arr[:,0]
```

</details>

