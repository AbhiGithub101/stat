...
quarter_sales = df.iloc[:, 1:]
adf = pd.DataFrame()
adf['Quarter'] = quarter_sales.columns
adf['total sales'] = quarter_sales.sum(axis=0).values
adf
```
...
```python
adf['maximum sales'] = quarter_sales.max(axis=0).values
adf
```
...
```python
adf['minimum sales'] = quarter_sales.min(axis=0).values
adf
```
...
```python
adf['average sales'] = quarter_sales.mean(axis=0).values
adf
```
...
```python
adf['median sales'] = quarter_sales.median(axis=0).values
adf
```
...
```python
adf['mode sales'] = quarter_sales.mode(axis=0).apply(
    lambda col: ', '.join(col.dropna().astype(str)),
    axis=0
).values
adf
```
...
```python
adf['reported stores'] = quarter_sales.count(axis=0).values
adf
```
...
```python
adf['highest total sales overall'] = adf['total sales'] == adf['total sales'].max()
adf
```
...
```python
adf['sales range'] = adf['maximum sales'] - adf['minimum sales']
adf
```
...
```python
df['total yearly sales'] = quarter_sales.sum(axis=1)
df
```
...
```python
df['highest sales value'] = quarter_sales.max(axis=1)
df['highest sales quarter'] = quarter_sales.idxmax(axis=1)
df
```
...
```python
df['lowest sales value'] = quarter_sales.min(axis=1)
df['lowest sales quarter'] = quarter_sales.idxmin(axis=1)
df
```
...
```python
df['average quarterly sales'] = quarter_sales.mean(axis=1)
df
```
...
```python
df['median sales'] = quarter_sales.median(axis=1)
df
```
...
```python
df['reported quarters'] = quarter_sales.count(axis=1)
df
```
...
mdf=df.loc[df['total']==df['total'].max()]
mdf.iloc[0,0]
```
